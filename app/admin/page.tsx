...
export default function AdminPage(){
  ...
  useEffect(()=>{
    supabase.auth.getUser().then(({data})=>{if(data.user){setUser(data.user);checkAdmin(data.user.id)}});
  ...
  async function checkAdmin(id:string){
    ...
    const ok=data?.role==="admin"; setIsAdmin(ok); if(ok) loadContent();
  }
  ...
  async function login(e:React.FormEvent){e.preventDefault();setMessage("Connexion…");const {error}=await supabase.auth.signInWithPassword({email,password});setMessage(error?error.message:"Connecté.")}
  async function signup(){setMessage("Création du compte…");const {error}=await supabase.auth.signUp({email,password});setMessage(error?error.message:"Compte créé. Si Supabase demande une confirmation par courriel, confirme-la puis reconnecte-toi.")}
  async function logout(){await supabase.auth.signOut();setMessage("")}

  async function saveSettings(){
    if(!settings)return; setMessage("Enregistrement…");
    const {error}=await supabase.from("site_settings").update({...settings,updated_at:new Date().toISOString()}).eq("id",1);
    setMessage(error?error.message:"Image de marque enregistrée ✓");
  }

  async function saveChallenge(){
    if(!selected)return; setMessage("Enregistrement…");
    const {error}=await supabase.from("challenges").update(selected).eq("id",selected.id);
  ...
  async function uploadImage(file:File,kind:"hero"|"logo"|"challenge"){
    ...
    const {error}=await supabase.storage.from("app-media").upload(path,file,{upsert:false});
    if(error){setMessage(error.message);return}
  ...
  if(!user)return <AdminShell><div className="login-card"><h1>Administration</h1><p>Connecte-toi pour modifier Sniff and Fun Challenge.</p><form onSubmit={login}><label>Courriel<input type="email" value={email} onChange={e=>setEmail(e.target.value)} required/></label><label>Mot de passe<input type="password" value={password} onChange={e=>setPassword(e.target.value)} required minLength={6}/></label><button type="submit">Se connecter</button><button type="button" className="secondary" onClick={signup}>Créer mon compte administrateur</button></form>{message&&<p className="message">{message}</p>}</div></AdminShell>;

  if(!isAdmin)return <AdminShell><div className="login-card"><h1>Compte connecté</h1><p>Ton compte existe, mais il n’a pas encore le rôle administrateur.</p><p>Dis-moi simplement <strong>« mon compte admin est créé »</strong> et je pourrai activer le rôle administrateur dans Supabase.</p><button onClick={logout}>Déconnexion</button>{message&&<p className="message">{message}</p>}</div></AdminShell>;

  return <AdminShell><header className="admin-header"><div><p className="eyebrow">SNIFF AND FUN</p><h1>Administration</h1><p>Modifie les textes, images et couleurs sans toucher au code.</p></div><div className="header-actions"><a href="/" target="_blank">Voir l’application</a><button onClick={logout}>Déconnexion</button></div></header>
